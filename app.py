...
def _build_threat_timeline_context(context: dict, actions: list[dict] | None = None, *, limit: int = 40) -> list[dict]:
    entries: list[dict] = []
    for alert in context.get("alerts", []) or []:
        entries.append({
            "timestamp": alert.get("timestamp") or "",
            "kind": "detection",
            "title": alert.get("title") or "Threat detection",
            "description": alert.get("description") or "Detection generated by SAFE telemetry.",
            "severity": str(alert.get("severity") or "low").lower(),
            "host_id": alert.get("host") or "",
            "stage": str(((alert.get("event") or {}).get("details") or {}).get("tactic") or ""),
            "href": f"/soc/hosts/{alert.get('host')}" if alert.get("host") else "/soc",
        })
    for incident in context.get("incidents", []) or []:
        host_id = str(incident.get("host_id") or "").strip()
        entries.append({
            "timestamp": incident.get("updated_at") or incident.get("opened_at") or "",
            "kind": "incident",
            "title": incident.get("playbook_label") or incident.get("title") or "Active incident",
            "description": incident.get("description") or "Incident requires analyst review.",
            "severity": str(incident.get("severity") or "medium").lower(),
            "host_id": host_id,
            "stage": incident.get("tactic") or incident.get("chain_summary") or "",
            "href": f"/soc/hosts/{host_id}" if host_id else "/soc/incidents",
        })
    for action in actions or []:
        host_id = str(action.get("host_id") or "").strip()
        status = str(action.get("status") or "pending").lower()
        entries.append({
            "timestamp": action.get("updated_at") or action.get("created_at") or "",
            "kind": "response_action",
            "title": str(action.get("action_type") or "response_action").replace("_", " ").title(),
            "description": f"Endpoint response action is {status.replace('_', ' ')}.",
            "severity": "high" if status in {"failed", "refused"} else "medium" if status in {"pending", "leased", "running"} else "low",
            "host_id": host_id,
            "stage": "Response",
            "href": f"/soc/hosts/{host_id}" if host_id else "/soc/response-center",
        })
    for host in context.get("hosts", [])[:20]:
        if not host.get("last_seen") or host.get("last_seen") == "unknown":
            continue
        host_id = str(host.get("host_id") or host.get("host_name") or "")
        entries.append({
            "timestamp": host.get("last_seen") or "",
            "kind": "host_state",
            "title": f"{host.get('host_name') or host_id} is {host.get('status', 'monitored')}",
            "description": f"Risk {host.get('risk_score', 0)}/100, protection {host.get('protection_state', 'monitored')}.",
            "severity": str(host.get("risk_level") or "low").lower(),
            "host_id": host_id,
            "stage": "Host State",
            "href": f"/soc/hosts/{host.get('host_name') or host_id}",
        })
    entries.sort(key=lambda item: str(item.get("timestamp") or ""), reverse=True)
    return entries[: max(1, min(int(limit), 100))]


def _build_campaign_context(context: dict) -> list[dict]:
    try:
    ...
    tenant_id = _current_request_tenant_id()
    events = []
    for event in (context.get("xdr_summary") or {}).get("recent_events", []) or []:
        item = dict(event)
    ...
    for incident in context.get("incidents", []) or []:
        item = dict(incident)
        item.setdefault("tenant_id", tenant_id)
        item.setdefault("event_type", "incident")
        item.setdefault("incident_id", incident.get("incident_id") or incident.get("id"))
        events.append(item)
    return [
        item.to_dict()
        for item in IncidentCorrelationV2(window_seconds=24 * 3600).correlate_events(events, tenant_id=tenant_id)
    ][:20]


def _build_soc_experience_context(base_context: dict | None = None) -> dict:
    context = base_context or _build_soc_preview_context()
    tenant_id = _current_request_tenant_id()
    try:
        actions = _get_agent_action_repository().list_actions(tenant_id=tenant_id, limit=200)
        ...
        actions = []
    try:
        from xdr.posture_engine import calculate_posture_score

        posture = calculate_posture_score(
            hosts=context.get("hosts", []),
            incidents=context.get("incidents", []),
            response_queue=(context.get("overview") or {}).get("response_queue", {}),
            telemetry=context.get("overview", {}),
        ).to_dict()
    except Exception:
        posture = {
            "score": 50,
            ...
            "risk_level": "high",
            "factors": {},
            "recommendations": ["review_security_overview"],
        }
    threat_hunts = _build_threat_hunts_payload(limit=250).get("hunts", [])
    campaigns = _build_campaign_context(context)
    threat_timeline = _build_threat_timeline_context(context, actions, limit=50)
    overview = dict(context.get("overview") or {})
    active_threats = int(overview.get("critical_alerts") or 0) + int(overview.get("critical_hosts") or 0)
    return {
        **context,
        ...
        "response_actions": actions[:100],
        "response_queue": {
            **_summarize_response_queue(actions, critical_hosts=int(overview.get("critical_hosts") or 0)),
            "mode": os.environ.get("NETGUARD_RESPONSE_MODE", "manual_approval"),
        },
        "soc_experience": {
            "active_threats": active_threats,
            "attack_campaigns": len(campaigns),
            "threat_hunts": len(threat_hunts),
            "timeline_events": len(threat_timeline),
        },
    }


def _build_executive_context() -> dict:
    context = _build_soc_experience_context()
    posture = context.get("security_posture") or {}
    overview = context.get("overview") or {}
    active_threats = int(overview.get("critical_alerts") or 0) + int(overview.get("open_incidents") or 0)
    if active_threats >= 3 or int(posture.get("score") or 0) < 45:
        business_impact = "Potential operational risk. Prioritize containment review and executive communication."
    elif active_threats:
        business_impact = "Security activity needs review, but no confirmed business disruption is visible."
    else:
        business_impact = "No active business-impacting incident is visible from current telemetry."
    return {
        **context,
        "business_impact": business_impact,
        "executive_recommendations": posture.get("recommendations") or ["maintain_monitoring_and_validate_coverage"],
    }
...
def _build_soc_host_detail_context(host_id: str, context: dict | None = None):
    ...
    try:
        from xdr.story_engine import build_incident_story

        host_story = build_incident_story(
            events=host_recent,
            detections=host_alerts,
            correlations=host_events,
            host_id=effective_host_id,
        ).to_dict()
    except Exception:
        host_story = {
            "summary": "SAFE is waiting for more endpoint telemetry to build an investigation story.",
            "progression": [],
            "impacted_assets": [effective_host_id],
...
def pricing():
    ...
    contact_email = CONTACT_EMAIL if BILLING_OK else "contact@safe.local"
...
def report_monthly():
    ...
    filename = f"safe-relatorio-{label}.pdf"
...
<div class="card">
  ...
  <div class="meta">
    Dúvidas? Entre em contato: <a href="mailto:contact@safe.local" style="color:#58a6ff">contact@safe.local</a>
  </div>
</div>
</body>
</html>"""
...
@require_session
def executive_page():
    audit("EXECUTIVE_VIEW", actor=_current_request_tenant_id(), ip=request.remote_addr or "-", detail="page")
    return render_template("executive.html", **_build_executive_context())
...
if "soc_views" not in app.blueprints:
    app.register_blueprint(
        create_soc_blueprint(
            ...
            build_soc_preview_context=_build_soc_experience_context,
